...
--7.2
Select Criminal_ID, LAST, FIRST, CLASSIFICATION, DATE_CHARGED, APPEALS.FILING_DATE, APPEALS.STATUS
From Criminals JOIN Crimes USING(CRIMINAL_ID), APPEALS
WHERE APPEALS.CRIME_ID = Crimes.CRIME_ID;
...
From ALIASES JOIN Criminals USING (CRIMINAL_ID);

--7.5
SELECT FIRST, LAST, START_DATE, END_DATE, (END_DATE - START_DATE) AS DURRATION, PROB_CONTACT.CON_FREQ AS CONTACT_FREQ
FROM CRIMINALS JOIN SENTENCES USING (CRIMINAL_ID), PROB_CONTACT
  WHERE SENTENCES.TYPE = 'P'
    AND (END_DATE - START_DATE) >= PROB_CONTACT.LOW_AMT
    AND (END_DATE - START_DATE) <= PROB_CONTACT.HIGH_AMT
ORDER BY FIRST, LAST, START_DATE;

--7.6
SELECT o.FIRST, o.LAST, m.FIRST MANAGER_FIRST, m.LAST MANAGER_LAST
